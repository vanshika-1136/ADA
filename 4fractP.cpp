...
// Partition function used by QuickSort to sort based on profit (arr[i][1])
int partition(double **arr, int low, int high, int m) {
    double pivot = arr[high][1];  // Using the profit for sorting
    int i = low - 1;
    for (int j = low; j <= high - 1; j++) {
        if (arr[j][1] >= pivot) {  // Sort in descending order by profit
            i++;
...
// QuickSort to sort items based on the profit (arr[i][1])
...
vector<double> fractionalKnapsack(double **arr, int n, int capacity) {
    ...
    cout << "Total profit: " << profit << " for " << i + 1 << " items." << endl;
    return List;
}
...
int main() {
    ...
    int m = 2;  // We have 3 columns (weight, profit, profit/weight ratio)
    ...
    cout << "Sorting by profit in decreasing order using QuickSort" << endl;
